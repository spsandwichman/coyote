...
static bool is_const_zero(FeInst* inst) {
    return inst->kind == FE_CONST && fe_extra(inst, FeInstConst)->val == 0;
}

static bool can_const_u5(FeInst* inst) {
    return inst->kind == FE_CONST && fe_extra(inst, FeInstConst)->val <= 0b11111;
}
...
static bool can_const_u16(FeInst* inst) {
    return inst->kind == FE_CONST && fe_extra(inst, FeInstConst)->val <= UINT16_MAX;
}

static bool can_subi_const_u16(FeInst* inst) {
    return inst->kind == FE_CONST && fe_extra(inst, FeInstConst)->val >= 0xFFFF0001ull;
}

static u16 as_subi_const_u16(FeInst* inst) {
    // imm = -x
    return (u16)-fe_extra(inst, FeInstConst)->val;
}
...
static u16 as_hi_u16(FeInst* inst) {
    return (u16)(fe_extra(inst, FeInstConst)->val >> 16);
}
...
FeInstChain xr_isel(FeFunc* f, FeBlock* block, FeInst* inst) {
    ...
    //     switch (fe_extra(inst, FeInstParam)->index) {
    ...
    //         fe_extra(addi, XrRegImm16)->reg = zero;
    //         fe_extra(addi, XrRegImm16)->imm16 = as_u16(inst);
    ...
    //         FeInst* zero = mach_reg(f, block, XR_REG_ZERO);
    //         fe_extra(subi, XrRegImm16)->reg = zero;
    //         fe_extra(subi, XrRegImm16)->imm16 = as_subi_const_u16(inst);
    ...
    //         fe_extra(lui, XrRegImm16)->reg = zero;
    //         fe_extra(lui, XrRegImm16)->imm16 = as_hi_u16(inst);

    //         FeInst* addi = create_mach(f, XR_ADDI, FE_TY_I32, sizeof(XrRegImm16));
    //         fe_extra(addi, XrRegImm16)->reg = lui;
    //         fe_extra(addi, XrRegImm16)->imm16 = as_u16(inst);
    ...
    //         FeInst* addi = create_mach(f, XR_ADDI, FE_TY_I32, sizeof(XrRegImm16));
    //         fe_extra(addi, XrRegImm16)->reg = binop->lhs;
    //         fe_extra(addi, XrRegImm16)->imm16 = as_u16(binop->rhs);
    //         return fe_chain_new(addi);
    //     } else {
    //         FeInst* add = create_mach(f, XR_ADD, FE_TY_I32, sizeof(XrRegReg));
    //         fe_extra(add, XrRegReg)->r1 = binop->lhs;
    //         fe_extra(add, XrRegReg)->r2 = binop->rhs;
    ...
    //         fe_extra(sub, XrRegImm16)->reg = binop->lhs;
    //         fe_extra(sub, XrRegImm16)->imm16 = as_u16(binop->rhs);
    //         return fe_chain_new(sub);
    //     } else {
    //         FeInst* sub = create_mach(f, XR_SUB, FE_TY_I32, sizeof(XrRegReg));
    //         fe_extra(sub, XrRegReg)->r1 = binop->lhs;
    //         fe_extra(sub, XrRegReg)->r2 = binop->rhs;
    //         return fe_chain_new(sub);
    //     }
    // case FE_IMUL: {
    //     FeInst* mul = create_mach(f, XR_MUL, FE_TY_I32, sizeof(XrRegReg));
    //     fe_extra(mul, XrRegReg)->r1 = binop->lhs;
    //     fe_extra(mul, XrRegReg)->r2 = binop->rhs;
    //     return fe_chain_new(mul);
    // }
    // case FE_IDIV: {
    //     FeInst* div = create_mach(f, XR_DIV_SIGNED, FE_TY_I32, sizeof(XrRegReg));
    //     fe_extra(div, XrRegReg)->r1 = binop->lhs;
    //     fe_extra(div, XrRegReg)->r2 = binop->rhs;
    ...
    //         FeInst* zero = mach_reg(f, block, XR_REG_ZERO);
    //         FeInst* add = create_mach(f, XR_ADD, FE_TY_I32, sizeof(XrRegReg));
    //         fe_extra(add, XrRegReg)->r1 = zero;
    //         fe_extra(add, XrRegReg)->r2 = binop->lhs;
    //         fe_extra(add, XrRegReg)->shift_kind = XR_SHIFT_LSH;
    //         fe_extra(add, XrRegReg)->imm5 = as_const_u5(binop->rhs);
    ...
    //         fe_extra(shift, XrRegReg)->r1 = binop->lhs;
    //         fe_extra(shift, XrRegReg)->r2 = binop->rhs;
    //         fe_extra(shift, XrRegReg)->shift_kind = XR_SHIFT_LSH;
    ...
    //     fe_extra(slti, XrRegImm16)->reg = sub;
    //     fe_extra(slti, XrRegImm16)->imm16 = 1;
    ...
    //     FeInst* subi = create_mach(f, XR_SUBI, FE_TY_I32, sizeof(XrRegImm16));
    //     fe_extra(subi, XrRegImm16)->reg = sp;
    //     fe_extra(subi, XrRegImm16)->imm16 = fe_extra(inst, FeInstStackAddr)->item->_offset;
    ...
    //     }
    //     FeInst* ptr = fe_extra(inst, FeInstLoad)->ptr;

    //     // very simple load for now, just use the imm pattern for now
    //     FeInst* load = create_mach(f, kind, inst->ty, sizeof(XrRegImm16));
    //     fe_extra(load, XrRegImm16)->reg = ptr;
    //     fe_extra(load, XrRegImm16)->imm16 = 0;

    //     return fe_chain_new(load);
    // }
    // case FE_STORE:
    // case FE_STORE_VOLATILE: {
    //     FeInst* ptr = fe_extra(inst, FeInstStore)->ptr;
    //     FeInst* val = fe_extra(inst, FeInstStore)->val;
    ...
    //     fe_extra(store, XrRegRegImm16)->r1 = ptr;
    //     fe_extra(store, XrRegRegImm16)->r2 = val;
    //     fe_extra(store, XrRegRegImm16)->imm16 = 0;

    //     return fe_chain_new(store);
    // }
    // case FE_BRANCH: {
    //     FeInst* condition = fe_extra(inst, FeInstBranch)->cond;
    //     FeBlock* if_true = fe_extra(inst, FeInstBranch)->if_true;
    //     FeBlock* if_false = fe_extra(inst, FeInstBranch)->if_false;
    //     ;
    ...
    //             fe_extra(result, XrRegImm16)->reg = cmp_eq->lhs;
    //             fe_extra(result, XrRegImm16)->imm16 = as_u16(cmp_eq->rhs);
    ...
    //             fe_extra(result, XrRegReg)->r1 = cmp_eq->lhs;
    //             fe_extra(result, XrRegReg)->r2 = cmp_eq->rhs;
    //             chain = fe_chain_append_begin(chain, result);
    //         }

    //         fe_extra(beq, XrRegBranch)->reg = result;
    //         fe_extra(beq, XrRegBranch)->dest = if_true;
    //         fe_extra(beq, XrRegBranch)->_else = if_false;

    //         return chain;
    //     } else {
    //         // just branch-not-equal (not-zero)
    //         FeInst* bne = create_mach(f, XR_BNE, FE_TY_VOID, sizeof(XrRegBranch));
    //         fe_extra(bne, XrRegBranch)->reg = condition;
    //         fe_extra(bne, XrRegBranch)->dest = if_true;
    //         fe_extra(bne, XrRegBranch)->_else = if_false;
    ...
    //     fe_extra(j, XrJump)->dest = fe_extra(inst, FeInstJump)->to;
...
void xr_final_touchups(FeFunc* f) {
    ...
    //     fe_extra(spill_lr, FeInst__MachStackSpill)->val = lr;
    //     fe_extra(spill_lr, FeInst__MachStackSpill)->item = lr_slot;
    ...
    //     fe_extra(stack_setup, XrRegImm16)->reg = sp;
    //     fe_extra(stack_setup, XrRegImm16)->imm16 = stack_size;
    ...
    //         fe_extra(reload_lr, FeInst__MachStackReload)->item = lr_slot;
    ...
    //         fe_extra(stack_destroy, XrRegImm16)->reg = sp;
    //         fe_extra(stack_destroy, XrRegImm16)->imm16 = stack_size;
    ...
    //                 fe_extra(addi, XrRegImm16)->reg = unop->un;
    ...
    //             fe_extra(spill, XrRegRegImm16)->r1 = sp;
    //             fe_extra(spill, XrRegRegImm16)->imm16 = fe_extra(inst, FeInst__MachStackSpill)->item->_offset;
    //             fe_extra(spill, XrRegRegImm16)->r2 = fe_extra(inst, FeInst__MachStackSpill)->val;
    ...
    //             reload->vr_out = inst->vr_out;
    //             fe_extra(reload, XrRegImm16)->reg = sp;
    //             fe_extra(reload, XrRegImm16)->imm16 = fe_extra(inst, FeInst__MachStackReload)->item->_offset;
    ...
    //     if (inst->kind == XR_J && fe_extra(inst, XrJump)->dest->flags == block->flags + 1) {
    //         fe_inst_remove_from_block(inst);
    //         continue;
    //     }
